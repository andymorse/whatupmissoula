...
| Albertsons | Flipp (no email) | Their own `albertsons.com/weeklyad` is a store-gated SPA that never finishes rendering headless, so for a while the ad reached the site by hand (download the PDF → `scripts/wum-drop` → `drops/Albertsons/`). It turns out the ad isn't on albertsons.com at all: Albertsons publishes through **Flipp**, whose backend is public, unauthenticated, and keyed by **postal code**. `flipp_fetch.py` lists the flyers for 59801, picks the one named "Weekly Ad" that's valid today (not the month-long "Big Book of Savings"), then rebuilds its pages — Flipp stores a flyer as one wide canvas of all pages side by side, cut into 256px tiles at six zoom levels, so we download the tiles, stitch the canvas, and slice it back into per-page images by the `pages` bounds. Those go through the same image→vision path as every other store, so Albertsons still lands in Top Steals. Tagged `kind: flipp`. **Uses the second-sharpest zoom, not the sharpest** — Flipp's full-size tiles come back with the flyer's layers vertically displaced (price badges printed over their own numerals); one level down is clean, still exceeds `ai.image_max_px`, and keeps each page to a single vision tile. The flyer detail endpoint also returns structured `items` with exact prices, but they carry no regular price, category, unit, or loyalty flag — the fields vision reads off the art — so we use the art, not the JSON. `drops/Albertsons/` still works as a fallback if Flipp changes. |
